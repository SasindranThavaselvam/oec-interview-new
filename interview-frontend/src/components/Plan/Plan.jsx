...
const Plan = () => {
  ...
  const [proceduresBackup, setProceduresBackup] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      setProceduresBackup(procedures);
  ...
  const handleSearchTextChange = (e) => {
    ...
    if (search) {
      if (proceduresBackup && proceduresBackup.length > 0) {
        var searchResult = [];
        if (isNaN(search)) {
          searchResult = proceduresBackup.filter((p) =>
            p.procedureTitle.toUpperCase().includes(search.toUpperCase()));
        }
        else {
          searchResult = proceduresBackup.filter((p) =>
            p.procedureId === parseInt(search));
        }
        setProcedures(searchResult);
      }
    }
    else
      setProcedures(proceduresBackup);
  };
              ...
              <div className="card-body">
                <div className="col">
                  <input type="text"
                    ...
                    value={searchText}
                    placeholder="Search by procedure title or id" />
                </div>
                <div className="row">
                  <div className="col">
                    <h4>Procedures</h4>
